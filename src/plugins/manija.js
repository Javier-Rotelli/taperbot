...
export const getTable = async (sheet, params, log) => {
  ...
  const tabla = await getCells(categoria.tabla).then((cells) => {
    const rows = range(Math.ceil(cells.length / 11)).map((x, i) =>
      cells.slice(i * 12, i * 12 + 12)
    );
    if (extendida) {
      return (
        "```" + table(rows.map((row) => row.map((cell) => cell.value))) + "```"
      );
    } else {
      return (
        "```" +
        table(rows.map((row) => row.slice(0, 6).map((cell) => cell.value))) +
        "```"
      );
    }
  });
...
const quienMeFalta = async (sheet, user, log) => {
  ...
  return await Promise.all([
    ...
    const userIndex = users.findIndex(
      (cell) => cell.value.trim() === user && cell.col === 25
    );
    if (userIndex === -1) {
      return "y vos quien sos?";
    }
    const username = users[userIndex + 1].value;
    const playerIndex = pendingMatches.findIndex(
      (cell) => cell.value.trim() === username && cell.col === 1
    );
    if (playerIndex === -1) {
      return "y vos quien sos?";
    }
...
export const getLetsPlayFriendlyMessage = function (rivals) {
  ...
  const customSingleRivalMessages = {
    aspero: ["<rivals> prepará los gluteos y bajá."],
  };
  const singleRival = getSingleRival(rivals);
  const messages = isSingleRival(rivals)
    ? !Object.keys(customSingleRivalMessages).indexOf(singleRival) > -1 ||
      Math.random() > 0.5
      ? singleRivalMessages
